...
export default function AdminDiscussionsPage() {
  ...
  const fetchThreads = useCallback(async () => {
    ...
    // Admin sees ALL threads across all batches - no batch filtering
  ...
  useEffect(() => {
    const channel = supabase
      .channel('admin-discussion-threads')
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('admin-discussion-replies')
  ...
  async function handleCreateTopic() {
    ...
    try {
      const res = await fetch('/api/discussions/topics', {
        ...
        body: JSON.stringify({ name: newTopicName.trim(), description: newTopicDesc.trim() || null, color: newTopicColor, batch_id: null }),
      })
      if (res.ok) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Manage all discussions across batches, moderate conversations, and engage with users</p>
        </div>
      ...
      <Dialog
        ...
        title="Create a Post"
        description="Share an announcement, resource, or start a discussion"
        footer={
          <>
