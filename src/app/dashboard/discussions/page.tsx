...
export default function StudentDiscussionsPage() {
  ...
  const fetchThreads = useCallback(async () => {
    ...
    setBatchId(profile?.batch_id || null)
    ...
    if (profile?.batch_id) {
      query = query.or(`batch_id.eq.${profile.batch_id},batch_id.is.null`)
    } else {
    ...
    const enriched: ThreadWithAuthor[] = threadData.map(t => ({
      ...t,
  ...
  async function handleReply() {
    ...
    if (error || !reply) {
      ...
      await supabase
        .from('discussion_threads')
        .update({
          reply_count: activeThread.reply_count + 1,
  ...
  async function toggleUpvoteThread(thread: ThreadWithAuthor) {
    ...
    if (thread.has_upvoted) {
      ...
      setThreads(prev => prev.map(t =>
        t.id === thread.id ? { ...t, has_upvoted: false, upvote_count: t.upvote_count - 1 } : t
      ))
      if (activeThread?.id === thread.id) {
        setActiveThread(prev => prev ? { ...prev, has_upvoted: false, upvote_count: prev.upvote_count - 1 } : null)
      }
      ...
      await supabase
        .from('discussion_upvotes')
        .insert({ user_id: userId, thread_id: thread.id })

      setThreads(prev => prev.map(t =>
        t.id === thread.id ? { ...t, has_upvoted: true, upvote_count: t.upvote_count + 1 } : t
      ))
      if (activeThread?.id === thread.id) {
        setActiveThread(prev => prev ? { ...prev, has_upvoted: true, upvote_count: prev.upvote_count + 1 } : null)
      }
  ...
  async function togglePin(thread: ThreadWithAuthor) {
    const { error } = await supabase.from('discussion_threads').update({ pinned: !thread.pinned }).eq('id', thread.id)
    if (error) toast.error('Failed')
    else {
      toast.success(thread.pinned ? 'Unpinned' : 'Pinned')
      if (activeThread?.id === thread.id) setActiveThread(prev => prev ? { ...prev, pinned: !prev.pinned } : null)
      fetchThreads()
    }
  }

  async function toggleLock(thread: ThreadWithAuthor) {
    const { error } = await supabase.from('discussion_threads').update({ locked: !thread.locked }).eq('id', thread.id)
    if (error) toast.error('Failed')
    else {
      toast.success(thread.locked ? 'Unlocked' : 'Locked')
      if (activeThread?.id === thread.id) setActiveThread(prev => prev ? { ...prev, locked: !prev.locked } : null)
      fetchThreads()
    }
  }
  ...
  async function toggleAnswer(reply: ReplyWithAuthor) {
    const { error } = await supabase.from('discussion_replies').update({ is_answer: !reply.is_answer }).eq('id', reply.id)
    if (error) toast.error('Failed')
    else {
      toast.success(reply.is_answer ? 'Unmarked' : 'Marked as answer')
  ...
  async function handleDelete() {
    ...
    if (deleteDialog.type === 'thread') {
      ...
      else { toast.success('Deleted'); setActiveThread(null); fetchThreads() }
    } else {
      ...
      if (error) toast.error('Failed to delete')
        ...
        if (activeThread) {
          fetchReplies(activeThread.id)
          await supabase.from('discussion_threads').update({ reply_count: Math.max(0, activeThread.reply_count - 1) }).eq('id', activeThread.id)
  ...
  if (activeThread) {
    ...
    const canResolve = activeThread.author_id === userId || isMod
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2">
                  ...
                  {isMod && (
                    <>
                      <span className="mx-1 text-gray-300">|</span>
                      <Badge variant="default" className="text-[10px] px-1.5 py-0"><Shield className="mr-0.5 h-2.5 w-2.5" />Mod</Badge>
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Ask questions, share ideas, and collaborate with your peers</p>
        </div>
      ...
      <Dialog
        ...
        description="Share a question, idea, or resource with your peers"
        footer={
          <>
