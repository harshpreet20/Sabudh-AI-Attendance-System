...
export default function StudentMessagesPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`dm-messages-${activeChannel.id}`)
      .on('postgres_changes', {
        event: 'INSERT',
        schema: 'public',
  ...
  async function searchContacts(query: string) {
    ...
    if (batch?.instructor_id) {
      const { data: teacher } = await supabase
        ...
        .eq('auth_user_id', batch.instructor_id)
        .ilike('full_name', `%${query}%`)
        .single()

      if (teacher) {
        results.push({
          id: teacher.auth_user_id,
  ...
  return (
    <div className="flex h-[calc(100vh-10rem)] flex-col lg:flex-row gap-4">
      <div className="flex flex-col w-full">
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description={searchQuery ? 'Try a different search term.' : 'Start a conversation with a teacher or classmate.'}
            action={
              !searchQuery ? (
                <Button onClick={() => setNewDialog(true)} size="sm">
                  <Plus className="mr-1 h-4 w-4" />
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <Badge variant={channel.other_role === 'teacher' ? 'default' : 'secondary'} className="text-[10px] px-1.5 py-0 shrink-0">
      ...
      <Dialog
        ...
        title="New Message"
        description="Search for a teacher or classmate in your batch"
      >
