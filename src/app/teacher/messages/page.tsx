...
export default function TeacherMessagesPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`dm-messages-teacher-${activeChannel.id}`)
      .on('postgres_changes', {
        event: 'INSERT',
        schema: 'public',
  ...
  async function searchContacts(query: string) {
    ...
    if (!query.trim() || batchIds.length === 0) {
      setContacts([])
      return
    }
    ...
    const { data: students } = await supabase
      ...
      .select('auth_user_id, full_name, profile_image_url, batch_id')
  ...
  return (
    <div className="flex h-[calc(100vh-10rem)] flex-col lg:flex-row gap-4">
      <div className="flex flex-col w-full">
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description={searchQuery ? 'Try a different search term.' : 'Start a conversation with any student in your batches.'}
            action={
              !searchQuery ? (
                <Button onClick={() => setNewDialog(true)} size="sm">
                  <Plus className="mr-1 h-4 w-4" />
      ...
      <Dialog
        ...
        title="New Message"
        description="Search for a student in your batches"
      >
